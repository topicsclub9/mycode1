"""
Arcana Data for Matrix Destiny Chart
...
def get_arcana_info(number):
    """
    Get detailed information about a specific arcana number.
    
    Args:
        number (int): Arcana number (1-22)
    
    Returns:
        dict: Arcana information or None if not found
    """
    return ARCANA_DATA.get(number)

def get_all_arcana():
    """
    Get all arcana information.
    
    Returns:
        dict: All arcana data
    """
    return ARCANA_DATA

def get_arcana_names():
    """
    Get a simple mapping of numbers to arcana names.
    
    Returns:
        dict: Number to name mapping
    """
    return {num: data["name"] for num, data in ARCANA_DATA.items()}

# Test the arcana data
if __name__ == "__main__":
    print("Testing Arcana Data:")
    print()
    
    # Test getting specific arcana
    arcana_1 = get_arcana_info(1)
    print(f"Arcana 1: {arcana_1['name']}")
    print(f"Description: {arcana_1['description']}")
    print(f"Keywords: {', '.join(arcana_1['keywords'])}")
    print()
    
    # Test getting all names
    names = get_arcana_names()
    print("All Arcana Names:")
